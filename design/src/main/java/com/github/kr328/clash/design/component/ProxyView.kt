...
class ProxyView(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val barWidth = config.delayBarWidth
        val barInset = config.delayBarInset
        val barLeft = config.layoutPadding + barInset
        val barRight = barLeft + barWidth
        val barTop = config.layoutPadding + barInset
        val barBottom = height - config.layoutPadding - barInset
        ...
        val delayCount = paint.breakText(
            state.delayText,
            false,
            (width - config.layoutPadding * 2 - config.contentPadding * 2)
                .coerceAtLeast(0f),
            null
        )
        ...
        // draw delay status bar (color coded by latency, with pulse alpha during testing)
        paint.style = Paint.Style.FILL
        paint.color = state.delayColor
        paint.alpha = (state.delayBarAlpha * 255).toInt().coerceIn(0, 255)
        canvas.drawRoundRect(
            barLeft, barTop, barRight, barBottom,
            config.delayBarRadius, config.delayBarRadius, paint
        )
        paint.alpha = 255  // reset for text drawing
        ...
        // draw delay
        canvas.apply {
            val x = width - config.layoutPadding - config.contentPadding - delayWidth
        ...
        // draw title
        canvas.apply {
            val x = barRight + barInset
            val y = config.layoutPadding +
                    (height - config.layoutPadding * 2) / 3f - textOffset
        ...
        canvas.apply {
            val x = barRight + barInset
            val y = config.layoutPadding +
                    (height - config.layoutPadding * 2) / 3f * 2 - textOffset
